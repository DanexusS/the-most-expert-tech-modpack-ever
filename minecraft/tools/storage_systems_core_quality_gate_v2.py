...
from upgrade_storage_systems_core_v2 import CHAPTER_PATH, FOCUS_ITEMS, quest_for_item
...
def task_types(block: str) -> list[str]:
    ...
    return re.findall(r'(?m)^\s*type:\s*"([a-z0-9_]+)"', block[start:end])
...
def main() -> int:
    ...
    for item_id in FOCUS_ITEMS:
        ...
        if types.count("item") < 1 or types.count("checkmark") != 1:
            failures.append(f"{item_id}: expected item task and exactly one acceptance task; found {types}")
            status = "FAIL"
        ...
        if en_length < 180 or ru_length < 180:
            failures.append(
                f"{item_id}: operational explanation too short (EN {en_length}, RU {ru_length})"
            )
            status = "FAIL"
        rows.append((item_id, quest_id, len(types), en_length, ru_length, status))
    ...
    if failures:
        ...
        lines.append("All selected storage quests require practical acceptance and include bilingual failure-oriented guidance.")
    lines.append("")
