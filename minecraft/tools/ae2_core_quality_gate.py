...
from upgrade_ae2_core import CHAPTER_PATH, FOCUS_ITEMS, quest_for_item

ROOT = Path(__file__).resolve().parents[1]
LANG_DIR = ROOT / "config" / "ftbquests" / "quests" / "lang"
REPORT_PATH = ROOT / "docs" / "AE2_CORE_QUALITY_REPORT.md"
...
def main() -> int:
    ...
    for item_id in FOCUS_ITEMS:
        ...
        if not en_title or not ru_title:
            ...
            status = "FAIL"
        if en_length < 120 or ru_length < 120:
            failures.append(f"{item_id}: description shorter than 120 characters")
            status = "FAIL"
        rows.append((item_id, quest_id, tasks, en_length, ru_length, status))

    lines = [
        "# Applied Energistics 2 Core Quality Report",
        "",
        "This report covers the redesigned network-design route inside the legacy Applied Energistics 2 chapter.",
        "",
    ...
    if failures:
        ...
        lines.append("**PASS** — all selected AE2 network gates have acceptance tasks and bilingual operational guidance.")
    lines.append("")
    REPORT_PATH.write_text("\n".join(lines), encoding="utf-8", newline="\n")

    print(f"ae2_core_quality: {'FAIL' if failures else 'PASS'}")
