...
from catalog_upgrade_common import parse_localization, task_types, visible_length
...
GENERATED_PREFIXES = ("main_stage_", "stage_annex_", "core_technology_manual_")
QUEST_ID_RE = re.compile(r'(?m)^\s*id:\s*"([0-9A-F]{16})"')
AUTHORIZED_REWARD_ITEMS = {
    "kubejs:divine_seal",
    "kubejs:eden_seal",
...
def unauthorized_reward(block: str) -> bool:
    if not re.search(r"(?m)^\s*rewards:\s*", block):
        return False
    try:
        _, _, body = extract_array(block, "rewards:")
    except RuntimeError:
        return True

    reward_types = re.findall(r'(?m)^\s*type:\s*"([a-z0-9_]+)"', body)
    reward_items = re.findall(
        r'(?ms)item:\s*\{.*?\bid:\s*"([a-z0-9_.-]+:[a-z0-9_./-]+)".*?\}',
        body,
    )
    return not (
        reward_types
        and len(reward_types) == len(reward_items)
        and all(kind == "item" for kind in reward_types)
        and all(item in AUTHORIZED_REWARD_ITEMS for item in reward_items)
    )
...
def main() -> int:
    ...
    for path in sorted(CHAPTER_DIR.glob("*.snbt")):
        ...
        for object_index, (start, end) in enumerate(quest_spans(text)):
            ...
            maximum_count = max(
                [int(value) for value in re.findall(r"\bcount:\s*([0-9]+)", block)],
                default=1,
            )
            flags: list[str] = []
            ...
            if unauthorized_reward(block):
                flags.append("HAS_REWARD")
            elif "rewards:" in block:
                authorized_reward_quests += 1
            if maximum_count > 64:
    ...
    duplicate_lookup = {owner for owners in duplicate_groups for owner in owners}
    ...
    rewrite_count = sum(row["classification"] == "REWRITE_REQUIRED" for row in chapter_rows)
    review_count = sum(row["classification"] == "REVIEW_REQUIRED" for row in chapter_rows)

    queue = {
        "schema_version": 3,
        "generated_exclusions": list(GENERATED_PREFIXES),
        "authorized_reward_items": sorted(AUTHORIZED_REWARD_ITEMS),
        "summary": {
            "legacy_chapters": len(chapter_rows),
            "legacy_quests": len(quest_rows),
            "malformed_quest_objects": malformed_total,
            "rewrite_required_chapters": rewrite_count,
            "review_required_chapters": review_count,
            "authorized_reward_quests": authorized_reward_quests,
    ...
    lines = [
        ...
        f"- Chapters requiring rewrite: **{rewrite_count}**",
        f"- Chapters requiring review: **{review_count}**",
        f"- Authorized seal-reward quests: **{authorized_reward_quests}**",
    ...
    for row in chapter_rows:
        flags = row["flags"]
        lines.append(
            f"| `{row['chapter']}` | {row['quests']} | {row['risk_score']} | {row['high_risk_quests']} | "
            f"{flags.get('MALFORMED_QUEST_OBJECT', 0)} | {flags.get('ITEM_ONLY', 0)} | "
            f"{flags.get('NO_OPERATIONAL_GUIDANCE', 0)} | {flags.get('HAS_REWARD', 0)} | {row['classification']} |"
        )
    ...
    for row in quest_rows[:100]:
        lines.append(f"| `{row['chapter']}` | `{row['quest_id']}` | {row['risk_score']} | {', '.join(row['flags']) or 'none'} |")
    lines.append("")
    ...
    print(f"rewrite_required: {rewrite_count}")
    print(f"review_required: {review_count}")
    print(f"authorized_reward_quests: {authorized_reward_quests}")
