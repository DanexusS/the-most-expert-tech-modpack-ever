...
from upgrade_mekanism_core import CHAPTER_PATH, FOCUS_ITEMS, quest_for_item

ROOT = Path(__file__).resolve().parents[1]
LANG_DIR = ROOT / "config" / "ftbquests" / "quests" / "lang"
REPORT_PATH = ROOT / "docs" / "MEKANISM_CORE_QUALITY_REPORT.md"
...
def main() -> int:
    ...
    for item_id in FOCUS_ITEMS:
        ...
        if en_length < 140 or ru_length < 140:
            failures.append(f"{item_id}: description shorter than 140 characters")
            status = "FAIL"
        rows.append((item_id, quest_id, tasks, en_length, ru_length, status))

    lines = [
        "# Mekanism Core Quality Report",
        "",
        "This report covers the redesigned material, gas and ore-processing route inside the legacy Mekanism chapter.",
        "",
    ...
    if failures:
        ...
        lines.append("**PASS** — all selected Mekanism process gates have acceptance tasks and bilingual operational guidance.")
    lines.append("")
    REPORT_PATH.write_text("\n".join(lines), encoding="utf-8", newline="\n")

    print(f"mekanism_core_quality: {'FAIL' if failures else 'PASS'}")
