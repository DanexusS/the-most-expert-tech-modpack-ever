...
from upgrade_ie_core import CHAPTER_PATH, FOCUS_ITEMS, quest_for_item

ROOT = Path(__file__).resolve().parents[1]
LANG_DIR = ROOT / "config" / "ftbquests" / "quests" / "lang"
REPORT_PATH = ROOT / "docs" / "IE_CORE_QUALITY_REPORT.md"
...
def main() -> int:
    ...
    lines = [
        "# Immersive Engineering Core Quality Report",
        "",
        "This report covers the redesigned production-critical route inside the legacy Immersive Engineering chapter.",
        "",
    ...
    if failures:
        ...
        lines.append("**PASS** — all selected IE production gates have acceptance tasks and bilingual operational guidance.")
    lines.append("")
    REPORT_PATH.write_text("\n".join(lines), encoding="utf-8", newline="\n")

    print(f"ie_core_quality: {'FAIL' if failures else 'PASS'}")
